...
import { listUserVideos } from '../../services/dbService';
...
const Homepage = () => {
  ...
  const [blockedInfo, setBlockedInfo] = useState<{ totalVideos: number; currentWeek: number } | null>(null);
  ...
  const handleRegister = async () => {
    ...
    try {
      const { user, error: registerError } = await register(
        email.trim(),
        process.env.REACT_APP_TEMP_PASS,
      );
      ...
      if (!user) {
        setError('Accesso non riuscito. Riprova tra qualche minuto.');
        return;
      }
  ...
  const blockedDescription = blockedInfo
    ? `Hai gia caricato ${blockedInfo.totalVideos} video. Il limite previsto per questa settimana e ${blockedInfo.currentWeek}.`
    : 'Non puoi accedere con questo account in questo momento.';
      ...
      <div className="page homepage">
        ...
        <div className="homepage__card">
          ...
          <div className="homepage__form">
            ...
            {error && (
              <span className="homepage__error" role="alert">
                {error}
              </span>
            )}
          </div>
